...
function matIV() {
  ...
  this.multiply = function (mat1, mat2, dest) {
    const a = mat1[0]; const b = mat1[1]; const c = mat1[2]; const d = mat1[3];
    const e = mat1[4]; const f = mat1[5]; const g = mat1[6]; const h = mat1[7];
    const i = mat1[8]; const j = mat1[9]; const k = mat1[10]; const l = mat1[11];
    const m = mat1[12]; const n = mat1[13]; const o = mat1[14]; const p = mat1[15];
    const A = mat2[0]; const B = mat2[1]; const C = mat2[2]; const D = mat2[3];
    const E = mat2[4]; const F = mat2[5]; const G = mat2[6]; const H = mat2[7];
    const I = mat2[8]; const J = mat2[9]; const K = mat2[10]; const L = mat2[11];
    const M = mat2[12]; const N = mat2[13]; const O = mat2[14]; const
      P = mat2[15];
  ...
  this.rotate = function (mat, angle, axis, dest) {
    ...
    if (!sq) { return null; }
    let a = axis[0]; let b = axis[1]; let
      c = axis[2];
    if (sq != 1) { sq = 1 / sq; a *= sq; b *= sq; c *= sq; }
    const d = Math.sin(angle); const e = Math.cos(angle); const f = 1 - e;
    const g = mat[0]; const h = mat[1]; const i = mat[2]; const j = mat[3];
    const k = mat[4]; const l = mat[5]; const m = mat[6]; const n = mat[7];
    const o = mat[8]; const p = mat[9]; const q = mat[10]; const r = mat[11];
    const s = a * a * f + e;
    const t = b * a * f + c * d;
    const u = c * a * f - b * d;
    const v = a * b * f - c * d;
  ...
  this.lookAt = function (eye, center, up, dest) {
    const eyeX = eye[0]; const eyeY = eye[1]; const eyeZ = eye[2];
    const upX = up[0]; const upY = up[1]; const upZ = up[2];
    const centerX = center[0]; const centerY = center[1]; const
      centerZ = center[2];
